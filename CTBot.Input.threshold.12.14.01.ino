...
/*   Issues
 *   erros on apicall , system restarting -> User exception (panic/abort/assert) v12.06  
  ...
  if ( millis() - previousMillis >= 1000UL) {    // one second, 'UL' means unsigned long
    ...
    if ( count60s >= 60 ) {
      ...
      if ( flagTgmDebug ==1 ) {  // testing Debug mode
        myBot.sendMessage(msg.group.id, "epochTime", "");
      } */
  ...
  if (epochTime > sunset) {
    ...
    if (epochTime < sunriseTomorrow) {
      ...
      if ( (getHoursNTP == 24) && (wateringUntilMidinghtFlag == 1) ) { // When it is midnight !
        ...
        apiCall();
        // return;   // maybe this is the issue
...
void apiCall() {
 ...
 //----------------- JSON ------------------------//
  ...
  yield(); // erros on restarting after apiCall , need to check this
}
